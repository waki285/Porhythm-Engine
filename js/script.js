...
function generateKeybindText(key) {
  return `<span class="judge-keybind">${key.replace("Key", "").replace("Arrow", "")}</span>`
}
...
class ViewController {
  constructor (gameController) {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      switch (e.code) {
        case keybinds[0]: { this.judgeEvent(1); break; } // a
        case keybinds[1]: { this.judgeEvent(2); break; } // s
        case keybinds[2]: { this.judgeEvent(3); break; } // d
        case keybinds[3]: { this.judgeEvent(4); break; } // f
        case keybinds[4]: { this.judgeEvent(5); break; } // g
      }
    });
    document.querySelectorAll('.btn-judge > div').forEach((el, index) => new JudgeBtn(el, index));
  }
  // 連続で処理が実行されないようにする
  btnAction (callback) {
...
class PlayView extends View {
  ...
  reset () {
    ...
    const keybinds = getKeybinds();
    document.querySelectorAll('.btn-judge > div').forEach((el, i) => el.innerHTML = generateKeybindText(keybinds[i]));
    this.hideEndText();
  }
  showEndText () {
...
class JudgeBtn {
  ...
  effect (text) {
    ...
    this.el.innerHTML = `${generateKeybindText(this.keybinds[this.index])}${html}`;
  }
  onClick () {
